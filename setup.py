...
extra_includes = []
import p2pool.futurocoin
import p2pool.networks
extra_includes.extend('p2pool.networks.' + x for x in p2pool.networks.nets)
import p2pool.futurocoin.networks
extra_includes.extend('p2pool.futurocoin.networks.' + x for x in p2pool.futurocoin.networks.nets)
...
try:
    ...
    if im64:
        bundle = bundle + 2
    sys.argv[1:] = ['py2exe']
    setup(name='p2pool-futurocoin',
        version=version,
        description='Peer-to-peer Futurocoin mining pool',
        author='Forrest Voight',
        author_email='forrest@forre.st',
        url='https://github.com/dashpay/p2pool-futurocoin/',
        data_files=[
            ('', ['README.md']),
...
dir_name = 'p2pool_futurocoin_win' + win + '_' + version
