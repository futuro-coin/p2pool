...
import p2pool
from . import data as futurocoin_data
...
class Protocol(p2protocol.Protocol):
    ...
    message_tx = pack.ComposedType([
        ('tx', futurocoin_data.tx_type),
    ])
    def handle_tx(self, tx):
        self.factory.new_tx.happened(tx)

    message_block = pack.ComposedType([
        ('block', futurocoin_data.block_type),
    ])
    def handle_block(self, block):
        block_hash = self.net.BLOCKHASH_FUNC(futurocoin_data.block_header_type.pack(block['header']))
    ...
    message_block_old = pack.ComposedType([
        ('block', futurocoin_data.block_type_old),
    ])
    ...
    message_headers = pack.ComposedType([
        ('headers', pack.ListType(futurocoin_data.block_type_old)),
    ])
    def handle_headers(self, headers):
        for header in headers:
            header = header['header']
            self.get_block_header.got_response(self.net.BLOCKHASH_FUNC(futurocoin_data.block_header_type.pack(header)), header)
