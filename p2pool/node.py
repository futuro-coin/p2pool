...
from p2pool.futurocoin import data as futurocoin_data, helper, height_tracker
...
class P2PNode(p2p.Node):
    ...
    def handle_shares(self, shares, peer):
        ...
        for share, new_txs in shares:
            if new_txs is not None:
                all_new_txs.update((futurocoin_data.hash256(futurocoin_data.tx_type.pack(new_tx)), new_tx) for new_tx in new_txs)
    ...
    def handle_bestblock(self, header, peer):
        if self.node.net.PARENT.POW_FUNC(futurocoin_data.block_header_type.pack(header)) > header['bits'].target:
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(share.header['previous_block']) > -5 or
                    self.node.futurocoind_work.value['previous_block'] in [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    def __init__(self, factory, futurocoind, shares, known_verified_share_hashes, net):
    ...
    def start(self):
        ...
        self.futurocoind_work = variable.Variable((yield helper.getwork(self.futurocoind, self.net)))
        @defer.inlineCallbacks
        def work_poller():
            while stop_signal.times == 0:
                flag = self.factory.new_block.get_deferred()
                try:
                    self.futurocoind_work.set((yield helper.getwork(self.futurocoind, self.net, self.futurocoind_work.value['use_getblocktemplate'])))
                except:
                    log.err()
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(futurocoin_data.block_header_type.pack(new_header)) <= self.futurocoind_work.value['bits'].target):
                return
            futurocoind_best_block = self.futurocoind_work.value['previous_block']
            if (self.best_block_header.value is None
                or (
                            new_header['previous_block'] == futurocoind_best_block and
                            self.net.PARENT.BLOCKHASH_FUNC(futurocoin_data.block_header_type.pack(self.best_block_header.value)) == futurocoind_best_block
                ) # new is child of current and previous is current
                or (
                            self.net.PARENT.BLOCKHASH_FUNC(futurocoin_data.block_header_type.pack(new_header)) == futurocoind_best_block and
                            self.best_block_header.value['previous_block'] != futurocoind_best_block
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.futurocoind_work.value['previous_block'])))
        self.futurocoind_work.changed.watch(lambda _: poll_header())
        ...
        self.get_height_rel_highest = yield height_tracker.get_height_rel_highest_func(self.futurocoind, self.factory, lambda: self.futurocoind_work.value['previous_block'], self.net)
        
        self.best_share_var = variable.Variable(None)
        self.desired_var = variable.Variable(None)
        self.futurocoind_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.futurocoind_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            added_known_txs = {}
            for tx_hash, tx in zip(self.futurocoind_work.value['transaction_hashes'], self.futurocoind_work.value['transactions']):
        ...
        # add p2p transactions from futurocoind to known_txs
        @self.factory.new_tx.watch
        def _(tx):
            self.known_txs_var.add({
                futurocoin_data.hash256(futurocoin_data.tx_type.pack(tx)): tx,
            })
        # forward transactions seen to futurocoind
        ...
        def _(share):
            ...
            if block is None:
                print >>sys.stderr, 'GOT INCOMPLETE BLOCK FROM PEER! %s futurocoin: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                return
            helper.submit_block(block, True, self.factory, self.futurocoind, self.futurocoind_work, self.net)
            print
            print 'GOT BLOCK FROM PEER! Passing to futurocoind! %s futurocoin: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
            print
    ...
    def set_best_share(self):
        best, desired, decorated_heads, bad_peer_addresses = self.tracker.think(self.get_height_rel_highest, self.futurocoind_work.value['previous_block'], self.futurocoind_work.value['bits'], self.known_txs_var.value)
	...
	#while i <= self.futurocoind_work.value['height']:
        ...
        if self.futurocoind_work.value['payment_amount'] >= 0 :
            real_pay = real_subsidy - self.futurocoind_work.value['payment_amount']
            return p2pool_data.get_expected_payouts(self.tracker, self.best_share_var.value, self.futurocoind_work.value['bits'].target, real_pay, self.net)
            
	if self.futurocoind_work.value['height'] > 158000+((576 * 30) * 17):
		real_pay = (real_subsidy)*40/100
        elif self.futurocoind_work.value['height'] > 158000+((576 * 30) * 15):
            real_pay = (real_subsidy)*42.5/100
        elif self.futurocoind_work.value['height'] > 158000+((576 * 30) * 13):
            real_pay = (real_subsidy)*45/100
        elif self.futurocoind_work.value['height'] > 158000+((576 * 30) * 11):
