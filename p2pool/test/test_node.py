...
from p2pool.futurocoin import data as futurocoin_data, networks, worker_interface
from p2pool.util import deferral, jsonrpc, math, variable

class futurocoind(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x132b9afeca5e9a2fdf4477338df6dcff1342300240bc70397c4bb: {
            ...
            'bits': futurocoin_data.FloatingInteger(bits=0x1b0d642e, target=0x44b9f20000000000000000000000000000000000000000000000L),
        }}
    ...
    def rpc_getblocktemplate(self, param):
        if param['mode'] == 'template':
            ...
            result = param['data']
            block = futurocoin_data.block_type.unpack(result.decode('hex'))
            ...
            if block['header']['previous_block'] != self.blocks[-1]:
                return False
            if futurocoin_data.hash256(result.decode('hex')) > block['header']['bits'].target:
                return False
            header_hash = futurocoin_data.hash256(futurocoin_data.block_header_type.pack(block['header']))
        ...
        for i in xrange(100):
            fee = i
            txs.append(dict(
                data=futurocoin_data.tx_type.pack(dict(version=1, tx_ins=[], tx_outs=[dict(value=fee, script='hello!' * 100)], lock_time=0)).encode('hex'),
                fee=fee,
            ))
        return {
            "version" : 3,
...
class MiniNode(object):
    ...
    def start(cls, net, factory, futurocoind, peer_ports, merged_urls):
        self = cls()
        
        self.n = node.Node(factory, futurocoind, [], [], net)
        yield self.n.start()
        ...
        wb = work.WorkerBridge(node=self.n, my_pubkey_hash=random.randrange(2**160), merged_urls=merged_urls, worker_fee=3)
        self.wb = wb
...
class Test(unittest.TestCase):
    ...
    def test_node(self):
        ...
        wb = work.WorkerBridge(node=n, my_pubkey_hash=42, merged_urls=[('http://127.0.0.1:%i' % (mm_port.getHost().port,), '')], worker_fee=3)
        web_root = resource.Resource()
        worker_interface.WorkerInterface(wb).attach_to(web_root)
